...
func _process(_delta: float) -> void:
	...
	if track_area_entered and grab_area_entered and Input.is_action_pressed("press_down"):
		if !initial_shift:
			...
			mouse_diff = clamp_grab_translation(mouse_diff)
			grab_area.translate(mouse_diff)
		else:
			initial_shift = false
...
func clamp_grab_translation(mouse_diff : Vector2) -> Vector2:
	if check_grab_translation(mouse_diff):
		return mouse_diff
	var clamped_x_mouse_diff = Vector2(0, mouse_diff.y)
	if check_grab_translation(clamped_x_mouse_diff):
		return clamped_x_mouse_diff
	var clamped_y_mouse_diff = Vector2(mouse_diff.x, 0)
	if check_grab_translation(clamped_y_mouse_diff):
		return clamped_y_mouse_diff
	return Vector2.ZERO
